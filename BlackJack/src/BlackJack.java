...
santiPath =  'C:\\Users\\LENVO USER\\PycharmProjects\\yer'
    ...
    if not os.stat(workerVacDatePath).st_size == 0:
        ...
        for i in range(len(workerVacDateArray)):
            ...
            with myExcelFile as csvfile:
                ...
                for row in reader:
                    if workerVacDateArray[i][0] != row[0] and workerVacDateArray[i][2] == row[2] and float(
                            workerVacDateArray[i][7]) < float(row[7]):
...
def refreshPage(windowRoot):


    try:
        if windowRoot.state() == 'normal':
            windowRoot.destroy()
    except NameError:
        pass
...
def deleteRowViewer(theViewer, path):
    ...
    try:
        ...
        refreshPage(vacationRVR)



        vacationRegularViewer()
...
def enterVacReqButton():
    ...
    if arrayName[userIndex] in workerVacHoursArray and hoursBool:
        print(userIndex)
        try:
...
def vacationRegularViewer():
    ...
    if not os.stat(workerVacHourPath).st_size == 0:
        try:
            loadWorkersVacationHours()
            ...
            reqDateChange = Button(vacationRVR, text='Request date change',
                                   command=lambda: swapRowViewer(vacationRV, workerVacDatePath))
            reqDateChange.pack()
...
def deleteRowCSV(deletedRow, path):
    ...
    with open('Temporary'+path, 'rt') as infile, open( path, 'wt') as outfile:
